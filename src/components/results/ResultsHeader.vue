...
<template>
    ...
    <div class="text-center">
        <template v-if="gameState.gameOverReason === 'ABANDONED'">
            <h2 class="text-5xl sm:text-6xl md:text-7xl lg:text-[5rem] font-black text-transparent bg-clip-text bg-gradient-to-b from-action-primary to-purple-500 drop-shadow-[0_5px_15px_rgba(168,85,247,0.5)] uppercase tracking-tighter mix-blend-screen animate-pulse">{{ t('results.victory') }}</h2>
            <div class="mt-6 bg-panel-card/80 backdrop-blur-md px-6 py-3 rounded-2xl inline-block border border-purple-500/30 shadow-[0_0_20px_rgba(168,85,247,0.2)]">
                <p class="text-purple-400 text-xl font-black uppercase tracking-widest">{{ t('results.byAbandon') }}</p>
                <p class="text-white/70 font-bold text-sm mt-1">{{ t('results.byAbandonDesc') }}</p>
            </div>
        </template>
        ...
        <div v-if="gameState.config.mode !== 'IMPOSTOR'" class="inline-block mt-4 bg-panel-base/60 backdrop-blur-xl border border-white/10 px-6 py-2.5 rounded-full shadow-lg">
            <p class="text-yellow-400 font-bold text-sm lg:text-base tracking-[0.4em] uppercase">{{ t('results.finalPodium') }}</p>
        </div>
    </div>
</template>
