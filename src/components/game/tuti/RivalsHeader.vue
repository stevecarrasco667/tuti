        ...
        <div class="flex flex-row flex-wrap items-center justify-center gap-6 lg:flex-col lg:flex-nowrap lg:overflow-y-auto lg:flex-1 lg:w-full lg:gap-3.5 pr-1 scrollbar-thin">
            ...
            >
                ...
                <div class="relative transition-transform group-hover:scale-110 flex-shrink-0">
                    <!-- Conic Progress Ring -->
                    <div 
                        class="absolute -inset-[3px] rounded-full p-[1.5px] transition-all duration-500 z-0"
                        :style="{
                    ...
                    <div v-if="rival.isActive && rival.isConnected && !rival.isAFK" 
                         class="absolute -inset-[3px] rounded-full border-2 border-tuti-teal animate-ping opacity-30 z-0">
                    </div>

                    <!-- Main Avatar Circle -->
                    <div class="w-14 h-14 lg:w-10 lg:h-10 rounded-full bg-panel-card border border-white/10 flex items-center justify-center text-2xl lg:text-lg shadow-sm relative z-10">
                        {{ rival.isConnected ? (rival.avatar || '👤') : '🔌' }}
                    </div>
                    
                    <!-- AFK Indicator -->
                    <div v-if="rival.isAFK && rival.isConnected" class="absolute -top-1 -right-1 z-20 text-[9px] bg-panel-card border border-white/20 rounded-full px-1.5 py-0.5 shadow-sm animate-bounce font-black" title="Ausente">💤</div>
                    
                    <!-- Active Typing Pencil Badge -->
                    <div v-if="rival.isActive && rival.isConnected && !rival.isAFK" 
                         class="absolute -bottom-1 -right-1 z-20 bg-tuti-teal text-[#0f0e2d] rounded-full w-4.5 h-4.5 flex items-center justify-center text-[9px] font-black shadow-[0_0_10px_rgba(45,212,191,0.6)] animate-bounce"
                         title="Escribiendo...">
                        ✍️
                    </div>
                </div>
                
                <div class="flex flex-col items-center lg:items-start flex-1 min-w-0">
                     <!-- Name (Desktop Only) -->
                    <span class="hidden lg:block text-sm font-black text-ink-main truncate w-full">{{ rival.name }}</span>
