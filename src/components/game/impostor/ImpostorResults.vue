    ...
    <div class="fixed inset-0 z-[100] flex flex-col items-center justify-center text-center overflow-hidden bg-black transition-colors duration-1000">
        ...
        <div class="z-10 flex flex-col items-center justify-between w-full h-full max-w-5xl md:max-w-[90%] mx-auto px-4 py-[clamp(0.5rem,2dvh,2rem)] relative">
            ...
            <div class="flex-1 flex flex-col items-center justify-center w-full gap-[clamp(0.5rem,2dvh,1.5rem)] pt-2 overflow-y-auto scrollbar-thin min-h-0">
                ...
                <h1 class="font-black uppercase tracking-tighter leading-[0.9] flex-none text-[clamp(2.5rem,8dvh,7rem)] w-full text-balance shrink-0"
                ...
                <p class="text-[clamp(1rem,3dvh,1.5rem)] text-white/80 font-black uppercase tracking-widest max-w-3xl mt-1 leading-tight shrink-0">
                    <template v-if="eliminatedPlayer">
                        El tribunal ha expulsado a <br/>
                        <span class="text-white font-black text-[clamp(2rem,6dvh,4rem)] mt-2 text-balance leading-none block shrink-0" :class="isEliminatedImpostor ? 'drop-shadow-[0_0_40px_rgba(106,215,229,0.8)]' : 'drop-shadow-[0_0_40px_rgba(239,68,68,0.8)]'">
                ...
                <!-- DETALLES FLOTANTES -->
                <div class="text-[clamp(1.2rem,4dvh,2.5rem)] font-black uppercase tracking-[0.3em] opacity-90 shrink-0">
                ...
                <div v-if="!matchOver" class="w-full max-w-2xl mt-2 flex flex-col gap-2 relative z-10 flex-none pb-4">
                    ...
                    >
                        <div class="flex items-center gap-2 min-w-0 flex-1">
                            <span class="text-[clamp(1rem,2.5dvh,1.5rem)] flex-shrink-0">{{ player.avatar || '👤' }}</span>
                            <span class="text-[clamp(0.75rem,2dvh,1rem)] font-black text-white/60 truncate uppercase tracking-wider">{{ player.name }}</span>
                            <span class="text-[clamp(0.75rem,2.5dvh,1.125rem)] font-black text-white ml-2 truncate">
            ...
            <div class="mt-auto flex-none flex flex-col items-center justify-end w-full shrink-0">
                <div v-if="matchOver" class="opacity-60 animate-pulse text-center w-full">
                    <p class="text-[clamp(0.5rem,1.5dvh,0.75rem)] text-white/50 uppercase tracking-[0.4em] font-black">Fin de la partida. Calculando veredicto...</p>
                </div>
                
                <div v-else class="text-[clamp(2.5rem,8dvh,4rem)] font-black font-mono animate-pulse drop-shadow-[0_0_30px_rgba(255,255,255,0.3)] leading-none" 
                     :class="timerColor">
