...
const crewWinners = computed(() => {
    ...
    // Supervivientes/Triunfadores reales que no eran impostores y siguen conectados
            ...
            <div class="w-full max-w-[1200px] mx-auto flex flex-col lg:grid lg:grid-cols-12 gap-10 lg:gap-16 items-start pb-8">
                ...
                <div class="w-full lg:col-span-7 xl:col-span-8 flex flex-col gap-10">
                    ...
                    <Transition enter-active-class="transition duration-1000 ease-out" enter-from-class="opacity-0 translate-y-8 scale-95" enter-to-class="opacity-100 translate-y-0 scale-100">
                        <div v-if="gameState.config.mode === 'IMPOSTOR' && showImpostorReveal" class="flex flex-col gap-6 mt-2 w-full max-w-3xl mx-auto">
                            
                            <!-- ESCENARIO A: Victoria Tripulante -->
                            <template v-if="gameState.impostorData?.cycleResult?.winner === 'CREW'">
                                <!-- 1. Héroes Sobrevivientes -->
                                <div class="bg-panel-card/80 backdrop-blur-md border-[3px] border-tuti-teal/40 rounded-3xl p-6 shadow-xl relative overflow-hidden">
                                    <h3 class="text-lg md:text-xl font-black text-center uppercase tracking-widest mb-6 text-tuti-teal drop-shadow-md">
                                        Tripulación Sobreviviente:
                                    </h3>
                                ...
                                <!-- 2. Impostores Perdedores -->
                                <div class="bg-action-error/5 backdrop-blur-md border-[2px] border-action-error/30 rounded-3xl p-5 shadow-inner">
                                    <h3 class="text-sm font-black text-center uppercase tracking-widest text-action-error/80 mb-4">
                                        Los Impostores descubiertos eran:
                                    </h3>
                            ...
                            <template v-else>
                                <div class="bg-panel-card/80 backdrop-blur-md border-[4px] border-action-error/50 bg-action-error/10 rounded-3xl p-8 shadow-[0_0_40px_rgba(239,68,68,0.15)] relative overflow-hidden">
                                    <h3 class="text-xl md:text-2xl font-black text-center uppercase tracking-widest mb-8 drop-shadow-md text-action-error">
                                        🦇 Los Impostores eran:
                                    </h3>
                            ...
                            <!-- Palabra Secreta Reveal Común -->
                            <div class="w-full text-center mt-2 animate-fade-in-up" style="animation-delay: 1.2s;">
                                <p class="text-ink-muted text-[10px] sm:text-xs uppercase font-black tracking-[0.3em] mb-3">La palabra secreta de la nave era:</p>
                                ...
                                <div v-if="gameState.impostorData?.cycleResult?.lastWishSuccess" class="mt-4 flex justify-center">
                                    <div class="inline-flex items-center gap-2 bg-action-error text-white font-black text-xs uppercase tracking-widest px-5 py-2 rounded-full shadow-[0_0_20px_rgba(239,68,68,0.6)] animate-bounce border border-action-error/50">
                                        <span>🃏</span>
                                        <span>¡Sobrevivieron adivinando la palabra!</span>
                                        <span>🃏</span>
                                    </div>
                                </div>
                            </div>
                    ...
                    <!-- PODIUM (Glassmorphism) Oculto en Modo Impostor -->
                    <div v-if="gameState.config.mode !== 'IMPOSTOR'" class="flex items-end justify-center gap-3 sm:gap-6 lg:gap-8 w-full mx-auto mt-6">
                        <!-- 2nd -->
